...
object DateSerializer : KSerializer<Date> {
    ...
    override fun serialize(encoder: Encoder, value: Date) {
        println(value.toFormattedDateString())
        encoder.encodeString(value.toFormattedDateString())
    }

    @RequiresApi(Build.VERSION_CODES.O)
    override fun deserialize(decoder: Decoder): Date {
        println(Date.valueOf(decoder.decodeString()))
        return Date(0)
    }
}
...
object TimeSerializer : KSerializer<Time> {
    ...
    override fun serialize(encoder: Encoder, value: Time) {
        encoder.encodeString(value.toFormattedTimeString())
    }
...
data class Medication(
    ...
    @SerialName("times") val times: MutableList<Time>
) {
    override fun toString(): String {
        return "Medication(pid=$pid, medication_id=$medicationId, amount='$amount', startDate=${startDate}, endDate=${endDate}, name=$name, notes=$notes) times=${times.toString()}"
    }
